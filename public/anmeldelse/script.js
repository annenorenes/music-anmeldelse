...
document.getElementById("skjema").addEventListener("submit", function (e) {
    e.preventDefault(); // hindrar at sida lastar på nytt

    const dato = document.getElementById("dato").value;
    const kommentar = document.getElementById("kommentar").value;
    const rating = document.getElementById("rating").value;

    // Hardkoda midlertidig – du kan gjere desse til felt seinare
    const bruker_id = 1;
    const utgivelses_id = 1;

    fetch("/api/skriv_anmeldelse", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        body: JSON.stringify({ dato, kommentar, rating, bruker_id, utgivelses_id })
    })
    .then(res => res.json())
    .then(data => {
        console.log("Svar frå server:", data);
